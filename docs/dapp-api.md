...
and consumed by the React UI and read-only tooling such as MetaMask.
...
Mutating `eth_*` methods (for example `eth_sendTransaction`) are intentionally
inactive. Clients must assemble and sign transactions locally, then POST
`/api/tx` with the unsigned inputs replaced by signatures.

### REST helpers

- `GET  /api/utxo?address=…` returns the spendable outputs (confirmed + pending)
  for a Base58 address.
- `POST /api/tx` accepts a fully signed transaction and submits it to the
  mempool.

The React dashboard relies on these endpoints to implement the browser wallet.
